...
mkdir -p "$INSTALL_DIR"
cp -r "$(dirname "$0")/sysmon" "$INSTALL_DIR/"

mkdir -p "$HOME/.local/bin"
cat > "$HOME/.local/bin/sysmon" <<'LAUNCHER'
#!/usr/bin/env bash
exec python3 -m sysmon "$@"
LAUNCHER
chmod +x "$HOME/.local/bin/sysmon"
...
sed -i "1a export PYTHONPATH=\"$INSTALL_DIR:\$PYTHONPATH\"" "$HOME/.local/bin/sysmon"

# Autostart desktop entry
echo "[4/4] Setting up autostart..."
mkdir -p "$HOME/.config/autostart"
cat > "$HOME/.config/autostart/sysmon.desktop" <<DESKTOP
[Desktop Entry]
Type=Application
Name=SysMon
Comment=System Monitor
Exec=$HOME/.local/bin/sysmon
...
cat > "$HOME/.local/share/applications/sysmon.desktop" <<DESKTOP
[Desktop Entry]
Type=Application
Name=SysMon
