...
class SysMonIndicator:
    def __init__(self, monitor, history, settings):
        ...
        if _HAS_INDICATOR:
            ...
            # On GNOME a single left-click opens this menu directly. Each metric
            # row carries a circular donut gauge icon, so one click shows the
            # usage gauges — no extra button to click.
    ...
    # ── Stats menu with donut-gauge icons ────────────────────────────────────
    ...
    def _build_menu(self) -> Gtk.Menu:
        ...
        self._mi_cpu = gauge_item()
        self._mi_gpu = gauge_item()
        self._mi_gpu.set_no_show_all(True)
        self._mi_ram = gauge_item()
    ...
    def _set_gauge(self, item, key, pct, label):
        ...
        img = item.get_image()
        if img is not None:
            img.set_from_file(gen_donut_icon(pct, key))
    ...
    def _refresh_menu(self, s: SystemStats, procs):
        ...
        if extra:
            cpu += "    " + "  ".join(extra)
        self._set_gauge(self._mi_cpu, "cpu", s.cpu_percent, cpu)
        ...
        if self.settings.show_gpu and s.gpu_available:
            ...
            self._set_gauge(self._mi_gpu, "gpu", s.gpu_percent, gpu)
        else:
            self._mi_gpu.set_visible(False)

        # Memory
        self._set_gauge(
            self._mi_ram, "ram", s.ram_percent,
    ...
    def _on_stats(self, s: SystemStats):
        ...
        # Refresh the menu so it is current whenever the user opens it.
        try:
